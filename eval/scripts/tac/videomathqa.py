"""
Evaluate VideoMathQA-style MCQ with three lenses:
  1) lmms_eval_accuracy: compare videomathqa_perception_score.pred_answer (letter) to GT letter (doc.answer).
...
Expected JSONL schema per line:
{
  "doc": {
    "question": "...",
    "options": ["A. 24", "B. 28", ...],   # list of strings with leading letters
    "answer": "B",
    ...
  },
  "videomathqa_perception_score": {"pred_answer": "E", ...},
...
def parse_options_list(option_lines: List[str]) -> List[str]:
    """
    From ["A. 24", "B. 28", ...] -> pure texts ["24","28",...], preserving original order.
    Non-matching lines are kept as-is (robustness).
    """
    out = []
...
def letters_from_options(option_lines: List[str]) -> List[str]:
    """
    Extract letters in order from ["A. 24", "B. 28", ...] -> ["A","B",...].
    If a line lacks a letter, assign incrementally from A..Z to keep list length consistent.
    """
    letters = []
    nxt = 0
    for s in option_lines:
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser(description="Evaluate VideoMathQA-style MCQ with LLM parsing of <think>/<answer>")
    ...
    for rec in tqdm(records, desc="Processing samples"):
        ...
        # Model's letter (for lmms_eval) comes from videomathqa_perception_score.pred_answer
        parsed_pred_letter = ((rec.get("videomathqa_perception_score") or {}).get("pred_answer") or "").strip().upper() or None
    ...
    for i, slot in enumerate(temp_slots):
        ...
        per_samples.append({
            ...
            "option_letters": slot["opt_letters"],      # e.g., ["A","B","C","D","E"]

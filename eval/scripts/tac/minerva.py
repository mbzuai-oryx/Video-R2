"""
Evaluate Minerva-style MCQ with three lenses (schema-specific):
  1) lmms_eval_accuracy: compare minerva_perception_score.pred_answer (letter) to GT letter (doc.answer).
...
Expected JSONL schema per line:
{
  ...
  "minerva_perception_score": {"question_id": 0, "duration": "medium", "category": "Basketball", "pred_answer": "A", "answer": "C"},
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser(description="Evaluate Minerva-style MCQ via LLM parsing of <think>/<answer>")
    ...
    for rec in tqdm(rows, desc="Processing samples"):
        ...
        # Model's letter for lmms_eval from minerva_perception_score.pred_answer
        mps = rec.get("minerva_perception_score") or {}

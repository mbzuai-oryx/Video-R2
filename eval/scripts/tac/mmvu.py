...
Evaluate MMMU-style video QA outputs with three accuracy lenses:
  1) lmms_eval_accuracy: compare model's parsed_pred to ground truth.
...
Adapted for JSONL records like:
{
  "doc": {
  ...
  "target": "A",
  "accuracy": {"pred_answer": "A"},
  "filtered_resps": "<think>...</think><answer>...</answer>"  # or nested list as before
}
"""
...
def is_mcq(doc: Dict[str, Any]) -> bool:
    qtype = (doc.get("question_type") or "").lower()
    if qtype == "multiple-choice":
        return True
    # Fallback: presence of choices
...
def best_from_parsed_pred(parsed_pred: Any, options: Optional[List[str]]) -> Tuple[Optional[str], Optional[str]]:
    """Return (mcq_letter, open_text) from a generic parsed_pred value."""
...
# ---- LLM parsing of <answer> text for answer_parsed_accuracy ----
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate MMMU-style Video QA with multiple parsing lenses (choices dict schema)")
    ...
    for rec in tqdm(records, desc="Processing samples"):
        ...
        # Build choices list ordered by letter A..Z if present
        choices_map = doc.get("choices") or {}
        if isinstance(choices_map, dict):
            letters_sorted = sorted(choices_map.keys())
            # Keep only A..Z and in order
            letters_sorted = [L for L in letters_sorted if isinstance(L, str) and len(L) == 1 and "A" <= L <= "Z"]
            options = [choices_map[L] for L in letters_sorted]
        else:
            options = []  # fallback (shouldn't happen in this schema)
        ...
        # Ground-truth letter comes directly from doc.answer / target
        gt_letter = (doc.get("answer") or rec.get("target") or "").strip() if mcq_flag else None
        gt_answer = gt_letter if mcq_flag else (doc.get("answer") or rec.get("target"))

        input_text = rec.get("input", "")

        # Responses: accept string (new) or old nested list
        raw_response = ""
        ...
        # Parsed pred (for lmms_eval path) from new field; fallback to old schema
        parsed_pred_any = (
            (rec.get("accuracy") or {}).get("pred_answer")
            or (rec.get("mmmu_acc") or {}).get("parsed_pred")
        )

        # Compute lmms correctness
        lmms_is_correct = False
        lmms_pred_letter = None
        lmms_pred_text = None
        if mcq_flag:
            lmms_pred_letter, _ = best_from_parsed_pred(parsed_pred_any, options)
            lmms_is_correct = compare_mcq(lmms_pred_letter, gt_letter)
        else:
            _, lmms_pred_text = best_from_parsed_pred(parsed_pred_any, None)
        ...
        temp_slots.append({
            ...
            "question": question,
            "options": options,
            "options_map": choices_map if isinstance(choices_map, dict) else None,
            "gt_answer": gt_answer,
            "gt_letter": gt_letter,
    ...
    for i, (slot, ext) in enumerate(zip(temp_slots, extracted)):
        ...
        per_samples.append({
            ...
            "mmmu_parsed_pred": (slot["rec"].get("accuracy") or {}).get("pred_answer") or (slot["rec"].get("mmmu_acc") or {}).get("parsed_pred"),
    ...
    # Agreement: cases where answer and thinking extractions match (both correct or both wrong)

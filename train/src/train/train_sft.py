...
try:
    ...
    print(f"liger kernel is not available. {e}")
...
def count_parameters(model):
    trainable = sum(p.ds_numel if hasattr(p, "ds_numel") else p.numel() for p in model.parameters() if p.requires_grad)
    frozen = sum(p.ds_numel if hasattr(p, "ds_numel") else p.numel() for p in model.parameters() if not p.requires_grad)
